  ...
  return (
    <>
      <div className="h-full" style={{ overflowY: 'auto', overflowX: 'hidden' }}>
        <div className="inline-block min-w-full" style={{ position: 'relative', overflow: 'visible' }}>
          <div className="flex">
          ...
          {timeSlots.map((timeSlot, idx) => (
            <div key={timeSlot} className="flex" style={{ position: 'relative', overflow: 'visible' }}>
              ...
              {employees.map(employee => {
                ...
                // Check if this cell is covered by a time block that starts in a cell above
                const isCoveredByBlock = blocks.some(block => {
                  if (block.employeeId !== employee.id) return false;
                  const blockStartMinutes = getMinutesFromTime(block.startTime);
                  const blockEndMinutes = getMinutesFromTime(block.endTime);
                  const slotMinutes = getMinutesFromTime(timeSlot);
                  // Cell is covered if it's within the block's duration but not the start
                  return slotMinutes > blockStartMinutes && slotMinutes < blockEndMinutes;
                });
                ...
                return (
                  <div
                    ...
                    className={`flex-1 min-w-[140px] sm:min-w-[200px] border-b border-l p-1 h-[40px] hover-elevate cursor-pointer relative ${
                      dragOverCell?.employeeId === employee.id && dragOverCell?.timeSlot === timeSlot
                        ? 'bg-blue-100 dark:bg-blue-900 border-blue-300 dark:border-blue-700'
                        : isCoveredByBlock ? '' : 'bg-card'
                    }`}
                    style={{ overflow: 'visible', position: 'relative', zIndex: isCoveredByBlock ? 0 : 1 }}
                  ...
                  >
                    {cellBlocks.length === 0 && (
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity z-0">
